...
import { SetNewEventSchedule } from 'components/TaskEditor/set-new-event-schedule.jsx'
...
export const Block = ({ closeOverlay, taskname, taskdescription, info, start, end}) => {
    const { C, setC } = CopyValue()
    const [startSchedule, setStartSchedule] = useState({ time: `${moment(start).format('HH:mm')}`})
    const [endSchedule, setEndSchedule] = useState({ time: `${moment(end).format('HH:mm')}`})  
    ...
    const startdate = () => {
      ...
      const formattedDateTimeString = moment(startDate).format('[GMT]ZZ [(Eastern Standard Time)]');
    ...
    const enddate = () => {
      ...
      const formattedDateTimeString = moment(endDate).format('[GMT]ZZ [(Eastern Standard Time)]');
      const startdate1 = `${startyear}-${startmonth}-${startday}`
      const startdate = moment(startdate1, 'YYYY-MM-DD').format('ddd MMM yyyy');
      const starttime = `${startdate} ${starthour}:${startminute}:00 ${starttimezone} ${formattedDateTimeString}`;
      const datetimeString = `${starttime}`
      ...
      {(
        ...
        >
          ...
          >
            ...
            />
             <CopyIcon onClick={() => handleCopy()}style = {{maxHeight:"25px", maxWidth: "25px", marginRight:"10px"}}/>
             <DeleteIcon style = {{maxHeight:"25px", maxWidth: "25px", marginRight:"10px"}} 
              onClick = { () => handleDelete() }/>
             <CloseIcon style = {{maxHeight:"25px", maxWidth: "25px"}} onClick = { () => closeOverlay() }/>
            </div>
            <div>
            <div className='add-task__attributes--left' style = {{marginLeft: "13px"}}>
              <StartDatePickerComponent/>
              <MyTimePicker/>
              <h3 style = {{marginRight:"10px", marginLeft: "4px"}}>to</h3>
              <SetNewEventSchedule
                isQuickAdd={false}
                isPopup={true}
