...
import { getTodayTimeRanges } from './timeRanges'

const MAX_NUM_CHUNKS = 8 // 2h
const PRIORITY_RANGE = Object.freeze([1, 4])
/***
 * Range for time length before scaling is 1-64
 * ***/
const TIME_LENGTH_RANGE = Object.freeze([0, 6]) // scaled by log2
/***
...
export const scheduleCalendar = async (userId) => {
  try {
    ...
    const { nowRounded, today, dayRange, workRange } = getTodayTimeRanges(
      userData.sleepTimeRange,
      userData.workTimeRange,
    )
    console.log('nowRounded: ', nowRounded)
